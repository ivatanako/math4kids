...
@Component({
  selector: 'app-multiplication-component',
...
export class MultiplicationComponent {
  ...
  onDifficultyChanged(difficulty) {
    ...
    this.http.get<Questionaire[]>(this.baseUrl + 'api/Questionaire/Multiplication/' + difficulty + '/' + this.totalItems).subscribe(result => {
  ...
  onSave() {
    for (let result in this.resultsArray) {
      this.db.collection('multiplication-results').add(this.resultsArray[result]);
    }
    this.isQuizCompleted = false;
  }
}
